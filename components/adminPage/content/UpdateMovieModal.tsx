...
const UpdateMovieModal = ({
  ...
  useEffect(() => {
    const fetchApi = async () => {
      try {
        const movie = await Axios(`Movie/${movieId}`);

        //Add value into form
        const filterCategories = movie.data.categories.map(
          (val: any) => val.categoryId
        );

        form.setFieldsValue({
          Mark: movie.data.mark,
          Duration: movie.data.time,
          Viewer: movie.data.viewer,
          Description: movie.data.description,
          EnglishName: movie.data.englishName,
          VietnamName: movie.data.vietnamName,
          Trailer: movie.data.trailer,
          DateCreated: dayjs(movie.data.dateCreated, "YYYY/MM/DD"),
          Nation: movie.data.nation.nationId,
          Feature: movie.data.feature.featureId,
          Category: filterCategories,
        });
        setImageUrl(movie.data.thumbnail);
      } catch (error) {
        console.log(error);
      }
    };
